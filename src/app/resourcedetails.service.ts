...
export class ResourcedetailsService {
  ...
  searchResource(resourceTypeId: number): Observable<any> {
    return this.httpClient.get(environment.apiurl + "/resourcedetails-by-typeid/" + resourceTypeId);
  }

  searchResourceActive(resourceTypeId: number): Observable<any> {
    return this.httpClient.get(environment.apiurl + "/resourcedetails-typeid-isactive/" + resourceTypeId);
  }
}
